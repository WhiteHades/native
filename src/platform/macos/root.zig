...
test "macos native platform disables web views" {
    var system = testPlatformWithEngine(.system);
    try std.testing.expect(!MacPlatform.supportsNativeFeature(&system, .main_webview));
    try std.testing.expect(!MacPlatform.supportsNativeFeature(&system, .child_webviews));
    try std.testing.expect(MacPlatform.supportsNativeFeature(&system, .native_views));
}
