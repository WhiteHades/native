...
const GtkEventKind = enum(c_int) {
    ...
    appearance = 16,
    audio = 17,
    widget_accessibility_action = 18,
};
...
const GtkWidgetAccessibilityNode = extern struct {
    id: u64,
    has_parent_id: c_int,
    parent_id: u64,
    role: c_int,
...
extern fn native_sdk_gtk_update_widget_accessibility(host: *GtkHost, window_id: u64, label: [*]const u8, label_len: usize, nodes: [*]const GtkWidgetAccessibilityNode, node_count: usize) c_int;
...
fn gtkCallback(context: ?*anyopaque, event: *const GtkEvent) callconv(.c) void {
    ...
    switch (event.kind) {
        ...
        .widget_accessibility_action => if (widgetAccessibilityActionFromGtkEvent(event)) |action| {
            state.emit(.{ .widget_accessibility_action = action });
        },
    }
}

fn widgetAccessibilityActionFromGtkEvent(event: *const GtkEvent) ?platform_mod.WidgetAccessibilityActionEvent {
...
fn updateWidgetAccessibility(context: ?*anyopaque, snapshot: platform_mod.WidgetAccessibilitySnapshot) anyerror!void {
    ...
    var nodes: [platform_mod.max_widget_accessibility_nodes]GtkWidgetAccessibilityNode = undefined;
    for (snapshot.nodes, 0..) |node, index| {
        nodes[index] = gtkWidgetAccessibilityNodeFromPlatform(node);
    }
    if (native_sdk_gtk_update_widget_accessibility(self.host, snapshot.window_id, snapshot.view_label.ptr, snapshot.view_label.len, nodes[0..snapshot.nodes.len].ptr, snapshot.nodes.len) == 0) return error.ViewNotFound;
}

fn gtkWidgetAccessibilityNodeFromPlatform(node: platform_mod.WidgetAccessibilityNode) GtkWidgetAccessibilityNode {
    return .{
        .id = node.id,
...
test "linux widget accessibility action preserves typed payload" {
    const label = "canvas";
    const text = "updated";
    var event = std.mem.zeroes(GtkEvent);
    event.window_id = 7;
    ...
    const action = widgetAccessibilityActionFromGtkEvent(&event).?;
    ...
    const value_action = widgetAccessibilityActionFromGtkEvent(&event).?;
...
test "linux widget accessibility ABI preserves canonical node" {
    ...
    const node = gtkWidgetAccessibilityNodeFromPlatform(source);
