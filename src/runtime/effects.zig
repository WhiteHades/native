...
/// completion inline from `submit_fn`. Completion copies
/// into a result workspace allocated by `external` on the UI thread, so
/// it never calls the application's allocator from a worker. `cancel_fn`
/// is the same nonblocking control operation. `shutdown_fn` is called once by
...
pub fn Effects(comptime Msg: type) type {
    return struct {
        ...
        const SlotKind = enum(u8) { spawn, fetch, file, clipboard, external };

        const EntryKind = enum(u8) { line, exit, response, file, clipboard, external };
        ...
        fn reclaimSlots(self: *Self) void {
            for (&self.slots) |*slot| {
                switch (slot.state.load(.acquire)) {
                    ...
                    .draining => if (slot.kind != .external and slot.fetch_buffer == null and slot.collect_buffer == null) slot.state.store(.idle, .release),
                    else => {},
                }
            }
        }
